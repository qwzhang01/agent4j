...
- [x] 确认本路线对应的开发版本：`0.1.5`（2026-09-17 去 SNAPSHOT 发布翻转：23 处 pom + CHANGELOG 落档 + 文档版本行统一刷新；deploy 手动执行）。
...
- [x] 增加任务 Lease、租约过期和抢占规则。（Stage 8.1 落地任务级 lease 原语——`JdbcTaskQueue.claimNext` guarded UPDATE 单赢家、`requeueOrphaned` stale-RUNNING 接管、`RunLeases.renew` 心跳续约（LeaseHeartbeatTest 3/3）；batch 5（2026-09-17）补齐当日记录的两个真缺：① 任务级独立租约钟——DDL 加 `heartbeat_at` 列（照 batch 1 `tenant_id` 先例：0.1.4 未发布直接改 DDL）、claim 时与 started_at 一起打点、`heartbeat(taskId)` 原语（guard `status='RUNNING'`，非 RUNNING 返 false = 所有权判决）+ `heartbeatAt(taskId)` 查询；`requeueOrphaned` 判活从 started_at 改为 `COALESCE(heartbeat_at, started_at)`——从不续约的持有者按 claim 时间判活，行为与 8.1 逐字节一致（两个 8.1 既有测试因此把崩溃模拟改为回拨租约钟，只拨 started_at 在新语义下不算陈旧，这正是新语义在工作）；requeue 同时重置 `heartbeat_at=NULL`（新 claim 不继承死者的旧租约钟）；`ClaimedTaskHeartbeat` 持有者自动续约（fail-open 于存储故障——计数重试，fail-closed 于所有权判决——续约见行非 RUNNING 即判丢停跳）。② `TaskLeaseSweeper` 周期调用 `requeueOrphaned`（首扫立即执行清启动残留，存储错误计数重试不杀死清扫器——静默死掉的清扫器等于回到原 gap，sweeps/requeuedTotal/totalErrors 可观测计数）。新测试 `TaskLeaseHeartbeatTest` 14/14：claim 初始化租约钟、RUNNING 续约、非 RUNNING 拒绝（PENDING/终态）、心跳保活使长任务免遭误收、不续约按 claim 时间判、宽限窗容忍短暂沉默、停跳被接管并可重新 claim、持有者保活/失约检测/close 停跳且幂等/存储故障 fail-open 不误判丢租约、清扫器周期 requeue/存储故障存活、构造守卫拒绝非正参数。agent-scheduler 74/74 全绿（11+4+14+既有套件），全仓 23 模块 verify 零回归。残余 gap 更新（batch 7 2026-09-17）：心跳线程形态已补——`ClaimedTaskHeartbeat` 新增共享 `ScheduledExecutorService` 构造器（宿主一池多持有者，close 只取消自身 future 绝不关池，`SharedHeartbeatPoolTest` 4/4 锚定），legacy 每持有者一线程构造器保留兼容；仍开放：抢占规则只有 lease 过期接管一种（无优先级抢占——高优任务不能驱逐 RUNNING 低优任务）、租约钟无跨实例时钟偏移补偿（多机部署依赖 NTP））
...
- [x] 处理 resources、prompts、sampling、elicitation 的能力声明。（harness 6.2 batch 3，2026-09-17：initialize 握手把 server capabilities 解析为类型化 record `McpServerCapabilities`（tools/resources/prompts/logging 四旗标按字段存在性判定——2024-11-05 规范把它们类型化为能力对象如 `"logging":{}`，首版 `path().asBoolean()` 对对象恒 false 是真 bug，测试抓住后修复并容忍 boolean 简写）；客户端能力对象刻意留空——本客户端只消费 tools，声明未实现的能力（sampling/elicitation）是服务器可能据此行动的谎言；`supportsTools()` 双向守卫 tool 路径：capabilities 缺席保持旧语义（规范中该字段可选，tools 视为可用），声明了能力但无 tools 以清晰消息拒绝并指明声明了什么。**gap：resources/prompts 已解析未消费（仍是 tools-only 客户端）；sampling/elicitation 为 server→client 方向，客户端声明无**。McpServerCapabilitiesTest 9 锚定）
- [x] MCP 连接重启、退避、冷却和任务取消与 RunContext 对齐。（harness 6.2 batch 3，2026-09-17，两半均闭合：①取消接线——`McpToolAdapter.execute(JsonNode, RunContext)` 从 ctx 取 `cancellationToken` 传入 `McpClient.callTool(name, args, token)`，取消语义为协作式三检查：wire 前 check、每轮 receive 轮询前 check、receive IOException 出口改写（若取消已发生则结构化信号顶替超时故事）——实证架构事实：`McpTransport.receive(Duration)` 把阻塞 receive 交给单线程 executor 并 get(timeout)，取消无法中断阻塞中的 poll，只能等 poll 退出（响应到达/超时/异常）后被检查；命中抛 `RunCancelledException`（结构化信号非 IOException），绝不包装成可重试异常。为承载此信号，agent-core `DefaultToolExecutor` 修复真契约违背：catch 链最前 rethrow `RunCancelledException | RunDeadlineException`（此前 catch(Exception) 把取消吞成 `[ERROR]` 文本让模型自愈——违背框架自身 Stage 1.4 契约，CancellationSource javadoc/ContractAwareToolExecutor/ReActAgentLoop 均已定义结构化信号路径）。诚实边界：未实现 MCP 协议级 cancellation notification（服务器侧工作继续）。②重启退避——原 gap 注记「重启退避未做」为事实错误：`ManagedMcpClient.recoverAndRetry` + `McpRestartPolicy`（maxRestarts 窗口预算 + cooldownMs 冷却 + windowMs 静默重置）早已落地，本批勘误。McpClientCapabilityAndCancelTest 8 + McpToolAdapterCancellationTest 5 锚定，其中 midReceiveCancellationSurfacesAsStructuredSignal 以 500ms receive 窗口实证「wire 阻塞穿透取消窗、出口改写」语义）
- [x] 增加第三方 MCP Server 互操作测试。（JDK HttpServer 真实 HTTP 模拟 SSE 方言对端 5/5；**gap：非官方参考实现**）

### 6.3 A2A

- [x] Agent Card 增加身份、能力和版本可信来源。（harness 6.3 batch 4，2026-09-17：卡身份密码学落地。`A2ACardIdentity` 原语层（与 A2ASecurity 同纪律——只管签名/验签，信什么是宿主策略）：Ed25519 优先（JDK 15+ SunEC）、RSA-2048 兜底，算法从 key 族派生不由调用方选（能选算法的调用方离 "none" 只差一次 confused deputy）；真 bug 由新测试抓住——SunEC Ed25519 密钥 `getAlgorithm()` 报家族名 `EdDSA`，首版映射表精确匹配 `Ed25519` 直接炸 `unsupported card-signing key algorithm: EdDSA`，修复为按家族名收 `EdDSA`/`Ed25519`。签名走响应头 `X-Agent-Card-Signature`（`v1;keyId=<16hex>;alg=<JCA名>;sig=<base64url>`）覆盖 card body 原始字节：body 保持 spec 纯净（不懂此扩展的 spec 对端看到的就是一张普通卡）、无规范化步骤（签的到底是什么无歧义）；keyId=SHA-256(encoded) 前 16 hex——是指纹，供信任库匹配与日志指名，不是 PKI。`HttpA2AServer` 9 参构造器收 `KeyPair`（签名要私钥、指纹要公钥，天然成对；首版只收 PrivateKey 再反射 `getPublic()` 派生公钥，对 SunEC EdDSA 私钥不可靠，砍反射改直收 KeyPair）：配置后 well-known 每响应带签名头，null 保持 legacy 无签名卡，`cardSigningKeyId()` 暴露指纹；7 参构造器委托 9 参保 0.1.3 二进制兼容（japicmp 门过）。`HttpA2AClient` 4 参构造器收 `Set<PublicKey>` 信任库（Set.copyOf 防御拷贝）：非空时 discoverAgents fail-closed 四路全拒（无签名头/头畸形 parseHeader 返回 null/未知 keyId/验签失败，A2AHttpException 指名原因），信任库条目按 keyId 指纹匹配；空信任库保持 D7 legacy（自报卡=路由输入非信任输入），签了名的卡对未配置信任库的客户端照常可读；取卡改保留原始字节（`getCard` 返回 body+bytes+签名头，旧 string-only `getJson` 删除，引用清点零残留）。诚实边界：框架信任扩展非 A2A spec securitySchemes（无此扩展的 peer 既不签也不验，其卡可信度不变）；key 分发是宿主的事；v1 无吊销故事（短生命进程，吊销需 CRL/OCSP 底座）。测试 18：A2ACardIdentityTest 9（密钥对往返/指纹稳定与互异/篡改·他钥·畸形·null 签名拒绝/头编解码/六种畸形头拒绝/家族名映射含 EdDSA）+ A2ACardSignatureWireTest 9（回环 HTTP：签服务器带头且签名对 served 字节可验/无签名服务器不带头/信任客户端接受/信任库客户端拒无签名卡·拒未知 key/空信任库双 legacy 路径/cardSigningKeyId 指纹一致/他卡字节签名不验）；wire 套件刻意不带 a2a-it tag——纯 127.0.0.1 loopback 零外部依赖，与 MCP loopback 测试同款进本地默认套件（信任特性的 wire 路径本地零执行是覆盖表演；跨进程重传输 IT 仍留 tag 矩阵）；agent-mcp 144/144，全仓 23 模块 verify 绿。**gap：key 分发无 PKI/目录、无吊销故事；跨进程第三方 A2A 实现互操作仍未做（现仍为回环+模拟方言）**）
...
- [x] Model、Tool、Workflow、Memory、Sandbox、MCP、A2A 都创建有层级关系的 Span。（harness 4.4 2026-09-17：新增 `OtelBoundaryEventSpanAdapter` 消费 BoundaryEvent——agent.memory/agent.sandbox（含 escalate/refuse 变体）/agent.approval/agent.model.serving 四面 + OtelRunEventSpanAdapter 的 agent.run/agent.step（Workflow 面）+ OtelMetricsSpanAdapter 的 agent.model/agent.tool，共十种 span；batch 7 2026-09-17：MCP/A2A 面接线——BoundaryEvent 扩两族（McpToolCallFinished/Failed、A2ATaskSent/Failed）+ `McpToolAdapter`/`InProcessA2AClient` 边界发射器（旁路纪律、结构属性、legacy 构造器逐字节不变）+ 翻译器 agent.mcp/agent.a2a span（CLIENT kind），共十二种；gap：`HttpA2AClient` 发射器与跨 server trace context 透传未做）
...
- [x] Planner/Executor：结构化计划、计划校验、计划版本和计划恢复。（workflow `plan` 包：`Plan`（Step DAG 声明 + validate 拒绝未知/前向/自环/重复依赖 + planVersion 重建即新身份 + readySteps 前沿计算）+ `PlanExecutor`（降级为拓扑序线性链复用既有 `GraphRuntime`，黑板承载结果，completedFrom 支持部分恢复）；测试 PlanTest 5 + PlanExecutorTest 4 全绿。测试暴露并修复两个真 bug：Plan 自环漏检、diamond 扇出与单游标 runtime 冲突。计划级恢复于 gap-closure 批次（2026-09-17）做实：`StepExecutor.execute(step, plan)` 扩 throws Exception（step 可表达 PauseException 暂停）+ `Plan.rebuildWith(newSteps)` 兑现版本演进承诺（旧恒为 1）+ `RunManager.resume` 补 DEFINITION_VERSION_MISMATCH 版本漂移守卫（checkpoint 的 workflowVersion 字段此前存在但无消费者，仅 DurableRunManager 比对）；PlanResumeTest 3 全绿——fresh RunManager 从磁盘 checkpoint 恢复：已完成节点永不重跑、暂停节点重入一次、completedFrom 报告持久化真相、漂移定义被拒。诚实边界：恢复粒度是 last pause 而非 last node（E8 已钉死的既有语义）。）
...
- [x] Model Routing：按 Step、风险、预算、质量门动态选择模型。（既有 Stage 18 + E2：`BudgetAwareRouter`（预算维度）、`ComplexityRouter`（Step 维度）、`CascadeModelClient` + QualityGate（质量门维度）。「按风险」维度于 gap-closure 批次（2026-09-17）清偿：`RiskAwareRouter`（agent-observability/routing）以 `SideEffectLevel` 为信号源——DESTRUCTIVE/SIDE_EFFECT/UNKNOWN 工具暴露即 premium，NONE/READ_ONLY 即 cheap；absent-from-map（装配未分类）≠ UNKNOWN（对缺席数据路由等于伪造风险，无信号跳过）。诚实边界：pre-call 只能看到请求暴露的工具 schema（模型中途才决定调什么），代理信号方向性安全——讨论破坏性工具也走 premium，误伤钱不误伤正确性。RiskAwareRouterTest 8 全绿。初记「tool 层无 riskLevel 声明」为事实错误：agent-security 既有 `SideEffectLevel`（NONE/READ_ONLY/SIDE_EFFECT/DESTRUCTIVE/UNKNOWN）即风险谱系，无需 Tool Contract 扩展。）
- [x] Replay/Time Travel：从事件历史重放，不重复执行真实副作用。（workflow `plan` 包 `EventReplayer`：TOOL 结果从录制的 history 折叠、绝不重新执行工具；partial replay（无 Done）标记 anomaly 并保留已见文本；Done 后到达的事件标记 anomaly；finalAnswerOf 按「最后 assistant」语义收敛。EventReplayerTest 7 全绿；测试暴露并修复文本丢失真 bug（ModelCallStarted/Done/!done 三处 flush）。交互式 time-travel 于 gap-closure 批次（2026-09-17）清偿：`replayPrefix(events, maxSteps, uptoIndex)` 独家前缀折叠 + `PrefixReplay(state, doneWithinPrefix, anomalies)`——Done 前截断得 mid-run world（IDLE、partial 历史保留、刻意截断的 "history ended without Done" anomaly 刻意丢弃）、越界 fail-loud IndexOutOfBoundsException、uptoIndex==size 且无 Done 时破碎录制 anomaly 保留。EventReplayerTest 增至 13 全绿。）
- [x] Typed Agent Event：模型开始/结束、Tool 参数校验、审批、恢复和路由事件完整化。（`AgentEvent` 新增 5 事件：ModelCallStarted/ModelCallFinished 接线 ReActAgentLoop 四路径、ToolValidationRejected 接线治理链（[UNKNOWN_TOOL]/[INVALID_TOOL_ARGUMENTS] → stage="validation"）、ReflectionStarted/ReflectionFinished 由 ReflectiveAgent 发射；既有精确事件序列断言适配 3 处（事件契约扩展是有意的）。审批/权限边界于 gap-closure 批次（2026-09-17）清偿：`ReActAgentLoop.governanceRejectionOf` 扩识别 `[DENIED]` → stage="approval"、`[RATE_LIMITED]` → stage="permission"，经真实 GovernedToolExecutor 装配验证（GovernanceRejectionEventTest 5：DENY 策略/审批拒绝/无审批服务→approval，限流→permission，AUTO 无事件）。诚实边界：`[DENIED]` 三来源（策略拒/审批拒/无审批服务）从拒绝串不可区分，统一归 approval stage——宿主审计问的是「谁拒了这单」而非三选一细分。初记「permission/approval 层无实现」为事实错误：agent-security 既有 `GovernedToolExecutor` 治理四件套与 `ToolPolicy`（AUTO/REQUIRES_APPROVAL/DENY）、agent-core approval 包（ApprovalRequest/Decision/Store/JdbcApprovalStore）全部存在。恢复事件仍为诚实 gap：WAITING_APPROVAL 状态机存在但恢复语义事件未发，留待 approval 交互闭环批次。）
