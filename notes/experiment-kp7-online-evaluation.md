...
### 题 4：窗口从 100 扩到 10 万，哪个设计先崩（2026-09-12 补）

先崩的不是内存，是两件更结构性的东西——一件性能层，一件语义层。

性能层：快照在锁内做全量重算。`snapshot()` 是 `synchronized` 的，锁内做 `List.copyOf(runWindow)`（10 万行全拷贝）+ durations 排序（O(n log n)）+ 内容窗 stream 平均。窗口 100 时这些是微秒级；10 万时排序到百毫秒级，且 `onRun` / `accept` 与 `snapshot` 争的是同一把锁——快照持锁做重活的期间，数据摄入停摆。环形缓冲本身不崩：`ArrayDeque.pollFirst` 逐出是 O(1)，10 万个装箱 Integer 也就几 MB。修法三件：分位数换 t-digest 类增量结构（P50/P95 不再排序）、快照持锁只做引用拷贝、重算移出锁外。锁内做重活才是崩点，队列不是。

语义层：窗口口径被稀释。漂移基线是窗口均值，窗口 10 万意味着「基线」跨越 10 万 run 的历史——一夜回归的新数据在窗口里占比 1%，对均值的拉动可以忽略，绊线分辨率被窗口大小反比稀释（检测突变需要的最小占比 ≈ 1/窗口大小）。100 → 10 万不是扩容，是把「最近口径」偷换成「全历史口径」，换了语义。窗口是漂移的分辨率参数，该跟着「想看多快的变化」定，不该跟着「内存能装多少」定。

一句话收口：内存是线性成本，锁内重算是平方级停摆，口径稀释是语义塌方——三者的崩法不在同一个数量级，先崩的是后两个。

### 题 5：PSI 为什么用等频 10 桶不用等宽 10 桶（2026-09-12 补）

因为答案长度分布是右偏长尾，等宽切会让 PSI 在两个方向同时失明。

等宽（按区间宽度均分）的下场：绝大多数样本挤在最左 1–2 桶，长尾摊在右侧桶里但每桶近乎空。`PSI = Σ(a_i − b_i)·ln(a_i/b_i)` 的空桶项是灾难：基准桶占比 b_i ≈ 0 或当前桶占比 a_i ≈ 0 时，`ln(a/b)` 发散（实践里用 0.0001 之类的 floor，结果是一个巨大的伪贡献，测的是 floor 的选择而不是分布的变化）。且信息全在 1–2 桶里，其余 8 桶全在测噪音——尾部分辨率为零，长尾怎么动都看不见。

等频（按基准分布的分位数切，每桶基准占比恰 10%）的本质：桶边界 = 基准的 deciles，PSI 测的是「当前分布在基准分位数轴上的质量迁移」。右偏长尾被分位数自然压缩进高值桶，每桶基准占比稳定，`ln(a_i/b_i)` 每一项都在测真实的占比变化，空桶问题被结构性排除（基准侧每桶必有 10%）。

代价面（为什么这不是免费午餐）：等频要先拿到锚定窗样本算分位数，桶边界绑定基准——重锚后边界必须重算。这正是 PSI 与锚定基线天然配套、与滑动基线错配的又一个原因：滑动基线每窗都在换「基准」，等于每窗都要重切桶，等频的稳定性优势荡然无存。详见 `experiment-kp7-drift-baseline.md` §Part B。
