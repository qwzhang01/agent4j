...
## 附录补充：什么时候需要知识图谱 -- "找人救火"场景

> 前一个附录讲的是 v1 -> v2 的升级原因（keyword -> 向量）。这个例子讲 v2 -> v3 的升级原因（向量 -> 图谱）：当答案分散在多条记忆里、需要跨记忆拼接推理时，keyword 和向量都彻底失效，只有图谱能做。

### 场景：团队知识 Agent 的"找人救火"

#### 设定

公司内部有个知识 Agent（Stage 12 要做的频道共享 Agent）。过去几个月，团队成员在频道里的对话被 Agent 记成了 7 条独立的记忆：

```text
记忆1 [FACT]    订单服务是 2024 年从单体拆出来的
记忆2 [FACT]    小王是订单服务的 owner
记忆3 [FACT]    小李 review 过订单服务 80% 的 PR
记忆4 [FACT]    小王这周休假，下周一回来
记忆5 [EVENT]   订单服务上周发布了 2.3.1 版本
记忆6 [FACT]    内存泄漏的排查方法是先看堆 dump
记忆7 [EVENT]   2.3.1 版本引入了新的连接池
```

现在用户在频道里问：

> "订单服务报内存泄漏了，现在找谁能最快处理？"

人类秒答："先找小李，小王休假中。" 推理链是：

```text
内存泄漏 -> 是订单服务的 bug
         -> 订单服务 owner 是小王
         -> 但小王这周休假
         -> 小李 review 过 80% PR，对代码最熟
         -> 结论：找小李（或等周一）
```
...
```text
store.query(keyword="内存泄漏")
  -> 命中记忆6（"内存泄漏的排查方法是先看堆 dump"）
  -> 记忆7 可能擦边（"连接池"，不含"内存泄漏"四个字，不命中）

返回：[记忆6]
```

得到的是**排查方法**，不是**人**。为什么找不到人？

**记忆2"小王是订单服务的 owner"里，根本没有"内存泄漏"这四个字。**

这就是 keyword 的死穴：它只在单条记忆内部做字面匹配。答案需要的"人"的信息，散落在不含查询词的其他记忆里。

#### 向量检索：查"内存泄漏找谁处理"

```text
查询 "内存泄漏找谁处理" -> embedding -> 查询向量
  -> 跟记忆6（内存泄漏排查）语义最接近 ✓
  -> 跟记忆7（连接池/版本）语义较近 ✓
  -> 跟记忆2（"小王是订单服务owner"）语义距离远 ✗
  -> 跟记忆4（"小王休假"）语义距离远 ✗
  -> 跟记忆3（"小李 review PR"）语义距离远 ✗

返回：[记忆6, 记忆7]
```

向量比 keyword 强一点（能召回"连接池"这种语义相关的），但**依然找不到人**。

为什么？看这 7 条记忆在语义空间的分布：
...
**"找谁处理"这个问题本身是技术域的**，它的向量天然靠近技术类记忆、远离人事类记忆。但答案恰恰需要跨域拼装：技术域的 bug + 人事域的 owner + 人事域的休假状态。

> **向量的本质是"找相似"。但这个问题的答案不是"和问题相似的记忆"，而是"多条不相似的记忆拼成的推理链"。**

这是 keyword 和 vector 共同的天花板：它们都是**单条记忆内部**的检索，不能**跨记忆**做连接。
...
```text
节点：小王、小李、订单服务、2.3.1版本、内存泄漏问题、休假状态

边：
  (订单服务)-[:OWNER]->(小王)
  (小王)-[:ON_LEAVE {until: 下周一}]->()
  (小李)-[:REVIEWED {ratio: 80%}]->(订单服务)
  (订单服务)-[:RELEASED]->(2.3.1版本)
  (2.3.1版本)-[:INTRODUCED]->(新连接池)
  (内存泄漏问题)-[:BELONGS_TO]->(订单服务)
```
...
```cypher
// 找"谁能处理订单服务的内存泄漏"
MATCH (issue {type: "内存泄漏"})-[:BELONGS_TO]->(svc:Service)
MATCH (owner:Person)-[:OWNER]->(svc)
OPTIONAL MATCH (owner)-[:ON_LEAVE]->(leave)
WITH svc, owner, leave
MATCH (backup:Person)-[:REVIEWED]->(svc)
WHERE leave IS NOT NULL          // owner 休假中
RETURN backup                    // -> 小李
```
...
```text
内存泄漏问题 --属于--> 订单服务 --owner--> 小王 --休假中--> 不可用
                                --review 80%--> 小李 ✓ 备选
```

**每一步都是顺着边走**：问题属于哪个服务 -> 服务的 owner 是谁 -> owner 现在可用吗 -> 不可用谁顶上。三跳推理，图数据库毫秒级返回。

#### 三种方式的本质区别

| 方式 | 检索单位 | 这个场景的结果 |
|------|---------|--------------|
| Keyword | 单条记忆（字面匹配） | 找到排查方法，找不到人 |
| Vector | 单条记忆（语义相似） | 找到技术相关记忆，找不到人（人事域记忆语义不相似） |
| **Graph** | **多条记忆（边连接推理）** | **问题->服务->owner->休假->备选，全链推出小李** |

一句话说透：

> **Keyword 和 Vector 回答"哪条记忆跟问题像"；Graph 回答"哪几条记忆连起来能推出答案"。**

当答案分散在多条记忆里、需要跨记忆拼接时，前两者彻底失效，只有图谱能做。

### 判断口诀（升级版）

之前的口诀是"看几跳"。这个场景给出更本质的判断：

```text
问自己：答案藏在一条记忆里，还是藏在多条记忆的关系里？

  答案在单条记忆里
    "用户对什么过敏？"          -> 一条记忆就写着 -> keyword/vector 够

  答案在多条记忆的关系里
    "找谁能处理这个 bug？"       -> 需要 服务+owner+休假 三条拼接 -> 图谱
    "用户亲属的过敏史？"         -> 需要 用户+亲属+病史 三条拼接 -> 图谱
    "上周三用户到底过不过敏？"    -> 需要 事实+时间轴 推理 -> 图谱（双时间轴）
```

对应到框架的落地节奏：

```text
v1（现在）  单条记忆 keyword 检索   -- "用户对什么过敏"级别的查询
v2（Stage15）向量语义检索          -- 还是单条记忆，但换说法能匹配
v3（以后）  图谱关系推理           -- "找人救火"级别的跨记忆推理
```

### 补充：记忆怎么变成图 -- 图谱升级的另一半

上面那 7 条记忆是**对话里自动提取的扁平条目**（v1 的 MemoryEntry 形态），但图谱需要**结构化的节点和边**。从"小王是订单服务的 owner"这句话变成 `(订单服务)-[:OWNER]->(小王)` 这条边，需要**实体识别 + 关系抽取**--这是 LLM 很擅长但 v1 没做的事。

这也是 Zep/Graphiti 的完整管线：对话进来 -> LLM 抽取实体和关系 -> 增量更新图谱 -> 查询时图遍历。

所以图谱的完整升级路径是两步：

```text
第一步：存储换成图结构（MemoryStore -> 图 DB）
第二步：提取换成关系抽取（Extractor 从"抽条目"升级成"抽实体关系"）

这比 v1->v2（只换检索方式）的改动大得多
-- 这也是为什么图谱排在最后（v3）
```

### v1/v2/v3 升级全景图（三个附录串起来）

```text
检索能力演进（检索单位不变：单条记忆）：
  v1 keyword 字面包含   -- "过敏"匹配"过敏"
  v2 向量语义相似       -- "过敏"匹配"禁忌"
  ↑ 附录补充 2 讲的升级原因：记忆多了全量注入撑不住，要筛相关的

推理能力演进（检索单位升级：多条记忆连接）：
  v3 图谱关系推理       -- 多条记忆拼成推理链，跨实体跨时间
  ↑ 本附录讲的升级原因：答案分散在多条记忆里，单条检索彻底失效

一句话：
  v1->v2 解决"找得更准"（同一条记忆，换说法也能匹配）
  v2->v3 解决"连得起来"（多条记忆拼成答案）
```
