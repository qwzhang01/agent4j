...
状态：写侧生命周期分流已落地代码（agent-memory 109/109 全绿，已提交 a50028c）；读侧 embedding（第①步）已落地（2026-09-10，全仓 22 模块 BUILD SUCCESS）；对账环、分层注入、持久化未动
...
- ② 对账环（写入前召回旧账给 LLM 对比挑键）+ 时间戳三件套（invalidAt / validFrom / validAt）
