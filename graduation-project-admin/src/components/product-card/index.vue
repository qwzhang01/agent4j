<template>
  <t-card theme="poster2" :bordered="false">
    <template #avatar>
      <t-avatar size="56px">
        <template #icon>
          ...
          <t-image v-else :src="'data:image/png;base64,' + product.photo" fit="fill" />
        </template>
      </t-avatar>
    </template>
    <template #status>
    ...
    <template #content>
      ...
      <p class="list-card-item_detail--desc" v-html="product.profile"></p>
    </template>
    <template #footer>
    ...
    <template #actions>
      <t-dropdown :disabled="!product.isSetup" trigger="click" :options="[
        {
          content: '查看',
          value: 'manage',
