<template>
  <view class="diy-banner" :style="{ height: `${imgHeights[imgCurrent]}rpx` }">
    <!-- 图片轮播 -->
    <swiper :autoplay="autoplay" class="swiper-box" :duration="duration" :circular="true"
      :interval="itemStyle.interval * 1000" @change="_bindChange">
      <swiper-item v-for="(dataItem, index) in dataList" :key="index">
        <image lazy-load :lazy-load-margin="0" class="slide-image" :src="dataItem.cover" @click="onLink(dataItem.jumpTo)"
          @load="_imagesHeight" />
      </swiper-item>
    </swiper>
    <!-- 指示点 -->
...
export default {
  ...
  methods: {
    ...
    _bindChange(e) {
      ...
      const item = this.dataList[e.detail.current];
      this.$emit("change", item);
      uni.setNavigationBarColor({
        frontColor: '#ffffff',
        backgroundColor: item.colorHex ? item.colorHex : '#00acac',
        // animation: {
        //   duration: 400,
        //   timingFunc: 'easeIn'
        // }
