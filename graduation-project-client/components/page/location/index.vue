<template>
  <!-- 定位店铺 -->
  <view class="main-loc">
    <view v-if="storeInfo.name" class="diy-location" :style="locationStyle">
      <view class="inner" @click="onTargetLocation">
        <view class="location-input">
          <text class="store">
            <text class="name">{{ storeInfo.name }}</text>
            <text class="switch">[更多]</text>
            <text class="address">
              <text class="location-icon iconfont icon-dingwei">
...
export default {
  ...
  computed: {
    locationStyle() {
      return "background: linear-gradient(to bottom, " + this.bgColor + "," + this.bgColor + ")"
    }
  },
  /**
   * 组件的方法列表
