  ...
  <view class="grade-popup popup" catchtouchmove="true" :class="(value && complete) ? 'show' : 'none'"
    ...
    <view v-if="!isShowPay" class="layer attr-content" :style="'border-radius: 10rpx 10rpx 0 0;'">
      <view class="specification-wrapper">
        <scroll-view class="specification-wrapper-content" scroll-y="true">
          <view class="specification-header">
            <view class="specification-name"><text class="price" v-if="memberGrade.catchValue > 0">￥{{
              memberGrade.catchValue }}</text>充值送{{ memberGrade.name }}</view>
          </view>
          <view class="specification-content">
            <view class="grade-item" v-if="memberGrade.discount > 0">
              <view class="item-rule">
                <view class="title">买单折扣：</view>{{ memberGrade.discount }}折
              </view>
            </view>
            <view class="grade-item" v-if="memberGrade.speedPoint > 0">
              <view class="item-rule">
                <view class="title">积分加速：</view>{{ memberGrade.speedPoint }}倍
              </view>
            </view>
            ...
            <view class="grade-description">
              <view class="item-rule">
                ...
                <view>{{ memberGrade.benefitDesc ? memberGrade.benefitDesc : '暂无' }}</view>
              </view>
            </view>
          </view>
        </scroll-view>
      ...
      <view class="btn-wrapper">
        <view class="sure" @click="prePay">立即充值</view>
      </view>
      <!-- 页面结束 -->
    </view>
    <!-- 要购买的等级信息确认 end -->
            ...
            <view class="specification-content">
              ...
              <view class="pay-item">
                <view class="item-amount">
                  <view class="title">
                    实付金额：<text class="amount">￥{{ ((parseFloat(memberGrade.upgradeValue) -
                      parseFloat(useCouponInfo.amount))
                      >= 0 ? (parseFloat(memberGrade.upgradeValue) - parseFloat(useCouponInfo.amount)) : 0.0.toFixed(2))
                    }}</text>
                  </view>
                </view>
              </view>
...
<script>
import { charge as chargeApi } from '@/service/my'
...
export default {
  ...
  methods: {
    ...
    doBuy() {
      ...
      // 请求api
      chargeApi(parseFloat(this.memberGrade.upgradeValue) - parseFloat(this.useCouponInfo.amount)).then(result => {
        if (result) {
