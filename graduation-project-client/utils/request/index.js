...
$http.requestStart = options => {
  ...
  // 请求前加入Token
  options.header['Access-Token'] = store.getters.token ? String(store.getters.token) : '47b8bb06b0849da166d4e36afc341ea1756f914d'
  // options.header['Access-Token'] = store.getters.token ? String(store.getters.token) : 'guest'
  // 商户号
...
$http.dataFactory = async res => {
  ...
  // 判断数据是否请求成功
  // result.code [ 200正常 300未登录 400没有权限  500有错误  600提示错误]
  // 判断是否需要登录
  if (httpData.code == 300 || httpData.code == 400) {
    // 300也有可能是后端登录态到期, 所以要清空本地的登录状态
