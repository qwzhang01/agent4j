...
import { notice, readNotice } from '@/service/index'
 ...
 */
export const getMoreListData = (page, oldPage, current) => {
  // 如果是第一页需手动制空列表
  if (current == 1) {
    oldPage.records = [];
  }
  // 合并新数据
  oldPage.records = oldPage.records.concat(page.records);
  return oldPage;
}
    ...
    notice()
      .then(result => {
        if (result) {
          ...
          uni.showModal({
            title: result.title ? result.title : '友情提示',
            content: result.content,
            showCancel: false,
            success(res) {
              if (res.confirm) {
                // 将消息置为已读
                readNotice(result.id)
              }
            }
          })
        }
