...
public class SimpleAgent implements Agent {
    ...
    public SimpleAgent(AgentConfig config) {
        ...
        this.loop = registry != null
                ...
                : new ReActAgentLoop(new com.seven.agent.core.tool.DefaultToolExecutor(registry));
    }
    ...
    public String run(String userInput, AgentState state) {
        ...
        if (!state.getMessages().isEmpty()) {
            state.addMessage(com.seven.agent.core.model.ChatMessage.user(userInput));
        } else {
            // Fresh state: init with system prompt + user input
            state.addMessage(com.seven.agent.core.model.ChatMessage.system(config.getSystemPrompt()));
            state.addMessage(com.seven.agent.core.model.ChatMessage.user(userInput));
        }
        ...
        for (int i = messages.size() - 1; i >= 0; i--) {
            ...
            if (msg.role() == com.seven.agent.core.model.ChatRole.ASSISTANT && msg.content() != null) {
                return msg.content();
            }
        }
